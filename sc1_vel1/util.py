...
import operator
from preprocess import load_sample, obj_loader
...
def load_input_only(path_coarse, rest_pos):
    ...
    for dirName, subdirList, fileList in os.walk(path_coarse):
        for x in range(1, FOLDER_SIZE + 1):
            ...
            obj_parser(coarse_file, batch_delta_coarse, batch_coarse_vel, rest_pos)

    x_pos = np.array(batch_delta_coarse) 
    # x_coarse = np.array(batch_coarse_pos)
    x_vel = np.array(batch_coarse_vel)
    x_pred = np.concatenate((x_pos, x_vel), axis=2)
...
# extend dataset x y
def load_data(path_coarse, path_tracking, rest_pos, tdelta=False):
    batch_coarse = []
    batch_coarse_vel = []
    batch_fine = []
    batch_fine_vel = []
    ...
    for x in range(1, FOLDER_SIZE + 1):
        ...
        obj_parser(coarse_file, batch_coarse, batch_coarse_vel, rest_pos)
        obj_parser(fine_file, batch_fine, batch_fine_vel, rest_pos)


    # print "load data:", len(batch_coarse)
    # Trains the model for a fixed number of epochs 
    x_pos = np.array(batch_coarse) 
    x_vel = np.array(batch_coarse_vel)
    x_train = np.concatenate((x_pos, x_vel), axis=2)    # concatenate position and velocity
...
def obj_parser(file_name, batch_data_pos, batch_data_vel=[], rest_pos=[]):
    ...
    with open(file_name, "r") as f1:
        for line in f1:
            ...
            if s[0] == 'v':
                ...
                if rest_pos:
                    v = list(map(operator.sub, v, rest_pos[count]))
                vert.append(v)
                count += 1
            elif s[0] == 'nv':
    ...
    batch_data_pos.append(vert)
    batch_data_vel.append(vel)
...
def write_obj(inputs, v_dim, faces, obj_out):
    # write to new file
    with open(obj_out, "w+") as f:
        for x in range(0,v_dim):
